...
.data 0x10000023
	...
	tabella_indirizzi_peso:	#NB al posto di asciiz avrei potuto usare anche .byte, ricordando che il numero che segue è la codifica del carattere ASCII 
...
.text
	...
	DtoB:	addiu $s0, $s0, 2			#$s0 deve puntare al primo carattere della stringa in ingresso.
			...
			loopDtoB:	lbu $s5, 0($s0)			#Carico in $s5 il byte contenuto nella cella di indirizzo $s0. NB si parte dalla cella di
						...
						addu $a0, $s5, $0		#memoria di indirizzo piu' grande fino ad arrivare a 0x10000002, quella in cui è contenuta
												#la codifica ASCII della cifra decimale di peso piu' grande.
						jal checkASCIIDec		#Per rendere la cifra utilizzabile nelle operazioni aritmetiche, forzo a 0 i primi 24 bit e
						add $s5, $v0, $0
						add $a0, $s5, $0		#In $a0 ho la configurazione binaria che rapprensenta in valore assoluto un numero da 0 a 9.
						add $a1, $s3, $0  		#In $a1 ho il peso della cifra che sto valutando e che sarà esponente del fattore moltiplicativo individuato da moltiplicatore.
	...
	BtoD:	
			...
			loopBtoD:	lbu $s5, 0($s0)			#Carico in $s5 il byte contenuto nella cella di indirizzo $s0. NB si parte dalla cella di
						addu $a0, $s5, $0		#memoria di indirizzo piu' grande fino ad arrivare a 0x10000002, quella in cui è contenuta
												#la codifica ASCII della cifra decimale di peso piu' grande.
						jal checkASCIIBin				#Per rendere la cifra utilizzabile nelle operazioni aritmetiche, forzo a 0 i primi 24 bit e
						add $s5, $v0, $0		#invariati i rimanenti 8. es codifica binaria ASCII di 1 è 00110001 che diventa 00000001.
	...
	HextoD:	la $s7, tabella_indirizzi_HexDecoder	#in $s7 carico l'indirizzo della tabella 
			addiu $s0, $s0, 2			#$s0 deve puntare al primo carattere della stringa in ingresso.
			add $s2, $0, $0				#In $s2 ci sara' la dimensione della stringa in ingresso. Qui inizializzo.
			add $s3, $0, $0				#In $s3 ci sara' il peso della cifra. Qui inizializzo.
			add $s4, $0, $0				#In $s4 ci sara' la rappresentazione in valore assoluto del numero binario identificato dalla stringa
										#ASCII in ingresso.
			...
			add $s0, $v0, $0			#Conservo e faccio uso dei risultati della procedura.
			add $s2, $v1, $0
			sltiu $t0, $s2, 9			#Il massimo numero rappresentabile, 0xffffffff, ha 8 cifre, nell'ordine di 16^8.
			beq $t0, $0, err_dimensione
			...
			loopHextoD:	lbu $s5, 0($s0)
						...
						jal checkASCIIHex1
						add $a0, $v0, $0		#In $a0 ho la configurazione binaria che rapprensenta in valore assoluto un numero da 0 a 9.
						add $a1, $s3, $0  		#In $a1 ho il peso della cifra che sto valutando.
						...
						addiu $s0, $s0, -1		#Decremento l'indirizzo che puntera' al byte precedente in memoria.
						sltu $t0, $s3, $s2		#Confronto tra peso e dimensione per capire se si e' conclusa la determinazione del numero binario
						bne $t0, $0 loopHextoD	#in valore assoluto.
						add $a0, $s4, $0		#Una volta concluso il compito precedente è necessario convertire in base 10 il numero in binario.
						add $a1, $s2, $0		#Ogni cifra sarà codificata con il corripondente codice ASCII in modo da stampare il risultato come stringa.
						add $a2, $s1, $0		#In $a0 c'e' il valore binario del numero. In $a1 la dimensione della stringa. In $a2 l'indirizzo di output.
						li $a3, 10				#Carico in $a3 la base in cui converto il numero binario.
						li $t2, 9				#Mi serve per limitare a 8 iterazioni il ciclo nella procedura conversione o dovrei mettere8?.
						jal conversione			#2^30 è circa 10^9, il massimo divisore.
						j termine
	
	DtoHex:	addiu $s0, $s0, 2			#$s0 deve puntare al primo carattere della stringa in ingresso.
			add $s2, $0, $0				#In $s2 ci sara' la dimensione della stringa in ingresso. Qui inizializzo.
			add $s3, $0, $0				#In $s3 ci sara' il peso della cifra. Qui inizializzo.
			add $s4, $0, $0				#In $s4 ci sara' la rappresentazione in valore assoluto del numero binario identificato dalla stringa
										#ASCII in ingresso.
			...
			loopDtoHex:	lbu $s5, 0($s0)			#Carico in $s5 il byte contenuto nella cella di indirizzo $s0. NB si parte dalla cella di
												#memoria di indirizzo piu' grande fino ad arrivare a 0x10000002, quella in cui è contenuta
												#la codifica ASCII della cifra decimale di peso piu' grande.
						addu $a0, $s5, $0		#memoria di indirizzo piu' grande fino ad arrivare a 0x10000002, quella in cui è contenuta
												#la codifica ASCII della cifra decimale di peso piu' grande.
						...
						addiu $s0, $s0, -1		#Decremento l'indirizzo che puntera' al byte precedente in memoria.
						sltu $t0, $s3, $s2		#Confronto tra peso e dimensione per capire se si e' conclusa la determinazione del numero binario
						bne $t0, $0 loopDtoHex	#in valore assoluto.
						add $a0, $s4, $0		#Una volta concluso il compito precedente è necessario convertire in base 10 il numero in binario.
						add $a1, $s2, $0		#Ogni cifra sarà codificata con il corripondente codice ASCII in modo da stampare il risultato come stringa.
						add $a2, $s1, $0		#In $a0 c'e' il valore binario del numero. In $a1 la dimensione della stringa. In $a2 l'indirizzo di output.
						li $a3, 16				#Carico in $a3 la base in cui converto il numero binario.
						li $t2, 7				#Mi serve per limitare a 7 iterazioni il ciclo nella procedura conversione. 16^7 e'
						jal conversione			#l'ultima potenza di 16 rappresentabile. E' 2^28.
						j termine
	...
	moltiplicatore:	addi $sp, $sp, -12		#moltiplicatore prevede di usare dei registri da preservare. Se ne fa il riversamento nella pila
				...
				prodotto:
					...
					mfhi $t0				#Controllo che il fattore moltiplicativo sia rapprensentabile. ES: 9 * 10^9 non è rappresentabile.
					bne $t0, $0, err_carry
...
potenza_iniziale:	sltu $t0, $a0, $s0		#Voglio trovare la potenza della base appena > di cio' che c'e' in $a0 in modo tale dividerlo per la potenza appena <.
					...
					beq $t0, $0, avanti2	#NB quando $t1 arriva a $t2, b^$t1 è gia in $s0 e quindi devo fermarmi
					multu $s0, $a3
					mflo $s0
					j potenza_iniziale
			avanti:	divu $s0, $a3
			...
			avanti2:divu $a0, $s0			#NB se $t0 = 9 e non ho ancora trovato la potenza appena > significa che ho gia' il divisore.
					...
					addi $sp, $sp, -16		#Riverso $a0 nella stack perche' mi servira' anche dopo e questo, per convenzione, non è un registro che
					sw $a3, 12($sp)
					sw $a2, 8($sp)
					sw $a1, 4($sp)
					...
					bne $s0, $t3 avanti		#La conversione termina dopo la divisione per 1. Uso $t3 per controllare.
					add $v1, $a2, $a0
					lw $s1, 0($sp)
					lw $s0, 4($sp)
					lw $ra, 8($sp)
	...
	ASCIIconv:				sltiu $t0, $a0, 10
							...
							la $a1, tabella_indirizzi_HexASCIIconv	#ricordati nel programma chiamante di salvare $a1, $a2
							li $t1, 15								#15 - valore da 10 a 15 ci da indice della tabella degli indirizzi
							subu $a2, $t1, $a0						#per scrivere la codifica ASCII della cifra esadecimale
							sll $a2, $a2, 2
							addu $a2, $a2, $a1
							lw $a3, 0($a2)
							jr $a3
	...
	checkASCIIHex1:	sltiu $t0, $a0, 0x67		#0x66 e' la codifica esadecimale ASCII di f
					beq $t0, $0, err_carattere
					sltiu $t0, $a0, 0x61		#0x61 e' la codifica esadecimale ASCII di a
					bne $t0, $0, checkASCIIHex2
					j HexDecoder
			
	checkASCIIHex2:	sltiu $t0, $a0, 0x47		#0x46 e' la codifica esadecimale ASCII di F
					beq $t0, $0, err_carattere
					sltiu $t0, $a0, 0x41		#0x41 e' la codifica esadecimale ASCII di A
					bne $t0, $0, checkASCIIDec
					j HexDecoder
					
	checkASCIIDec:	sltiu $t0, $a0, 0x3a 		#0x39 e' la codifica esadecimale ASCII di 9.
					beq $t0, $0, err_carattere	
					sltiu $t0, $a0, 0x30 		#0x30 e' la codifica esadecimale ASCII di 0.
					bne $t0, $0, err_carattere
					andi $v0, $a0, 0xf
					jr $ra
					
	checkASCIIBin:	sltiu $t0, $a0, 0x32 		#0x31 e' la codifica esadecimale ASCII di 1.
					beq $t0, $0, err_carattere	
					sltiu $t0, $a0, 0x30 		#0x30 e' la codifica esadecimale ASCII di 0.
					bne $t0, $0, err_carattere
					andi $v0, $a0, 0xf
	...
	HexDecoder:		andi $v0, $a0, 0xf			#Inizializzo l'indice della tabella.
					sll $t0, $v0, 2
					addu $t0, $t0, $a1
					addi $t0, $t0, -4
					lw $t1, 0($t0)
